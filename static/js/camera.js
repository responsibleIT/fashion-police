...
async function startCamera() {
  try {
    ...
    if (videoDevices.length === 0) {
      throw new Error(
        "No camera devices found. Please check if your USB webcam is connected."
      );
    }
...
async function loadMoveNet() {
  if (!window.poseDetection) {
    showStatus(
      "MoveNet model not loaded. Check your internet connection.",
      "error"
    );
    return;
  }
  ...
  try {
    ...
    if (poses && poses.length > 0) {
      ...
      if (keypoints.length > 0) {
        ...
        if (isPoseTrigger) {
          if (scanningBool === false) {
            triggerScanningText(true);
          }
...
function checkCapturePose(pose) {
  ...
  const leftWristAtEyeLevel =
    leftWrist.y >= eyeLevelTop && leftWrist.y <= eyeLevelBottom;

  // Check if right wrist is at eye level
  const rightWristAtEyeLevel =
    rightWrist.y >= eyeLevelTop && rightWrist.y <= eyeLevelBottom;
...
const scanning = document.querySelector("h2");

const triggerScanningText = (bool) => {
  console.log(scanningBool, "before");

  scanning.dataset.show = `${bool}`;
  scanningBool = bool;
  console.log(scanningBool, "after");
